...
**Sealed operation grants:** a distinct sealed-executor principal may call only its bound agent's synchronous `/api/agents/{agent}/task/sealed` route with a signed `operation_grant`. The grant is excluded from persistence and never appended to the model prompt, subprocess argv, or model-visible environment. A body-aware gate accepts only the exact synchronous sealed request shape: one `Bash` tool, bounded turns, deterministic idempotency, no caller-supplied attribution headers, and no session, file, model, prompt, timeout, or result-injection overrides. Raw sealed-request keys and primitive types are checked before Pydantic normalization, and execution fails closed before persistence when its idempotency claim is unavailable. Both the public backend and inner agent server mask the field with `SecretStr`; the public boundary rejects asynchronous use before idempotency, upload, or execution persistence, and a synchronous sealed request requires immediate capacity instead of entering persistent overflow. The execution service forwards the secret and validated turn cap only to the dedicated internal `/api/task/sealed` endpoint, including every retry. Legacy images do not implement that endpoint, so they fail closed instead of ignoring an unknown grant on `/api/task`; current images reject grants on the ordinary endpoint. The sealed endpoint rejects missing grants, broadened task shapes, and unsupported runtimes before starting Claude through the fixed setuid-root `trinity-task-context` runner. Admission requires an exact root-owned artifact contract that binds the packaged operation monitor, trust anchors, wrappers, and source revision by SHA-256. The runner reads only the grant prefix from stdin, validates it through the contracted operation broker, creates a new process session, and installs a root-only context under `/run/trinity-task-context/<session-id>` bound to the live session leader's `/proc` start identity. It then scrubs the environment, projects only the selected model credential and transport variables, and drops the model child to credential-blind UID/GID 1001. The broker resolves only its caller's session context, so concurrent ungranted tasks cannot claim another task's capability; a parent-death signal terminates the session leader and stale directories are unusable after abnormal termination or PID reuse. The least-capable default and Codex/Gemini adapters reject sealed grants rather than silently exposing or ignoring them.
