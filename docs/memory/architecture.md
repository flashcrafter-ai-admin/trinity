...
**Sealed operation grants:** authenticated synchronous Claude `/api/task` requests may include an optional signed `operation_grant` that is excluded from persistence and never appended to the model prompt, subprocess argv, or model-visible environment. A connector-scoped key may reach this endpoint only for its bound agent, and a second body-aware gate accepts only the exact synchronous sealed request shape: one `Bash` tool, bounded turns, deterministic idempotency, no source-attribution headers, and no session, file, model, prompt, timeout, or result-injection overrides. Raw sealed-request keys and primitive types are checked before Pydantic normalization, and connector execution fails closed before persistence when its idempotency claim is unavailable. Both the public backend and inner agent server mask the field with `SecretStr`; the public boundary rejects asynchronous use before idempotency, upload, or execution persistence, and a synchronous sealed request requires immediate capacity instead of entering persistent overflow. The execution service forwards the secret and validated turn cap only to the dedicated internal `/api/task/sealed` endpoint, including every retry. Legacy images do not implement that endpoint, so they fail closed instead of ignoring an unknown grant on `/api/task`; current images reject grants on the ordinary endpoint. The sealed endpoint rejects missing grants, asynchronous or broadened task shapes, and unsupported runtimes before starting Claude through the fixed setuid-root `trinity-task-context` runner. That runner reads only the grant prefix from stdin, validates it through the root-owned packaged operation broker, creates a new process session for the Claude tree, and installs a root-only context under `/run/trinity-task-context/<session-id>` bound to the live session leader's `/proc` start identity. The broker resolves only its caller's session context, so concurrent ungranted tasks cannot claim another task's capability; a parent-death signal terminates the session leader and stale directories are unusable after abnormal termination or PID reuse. The runner drops the Claude child back to UID/GID 1000 with the remaining stdin containing only the task prompt and removes the context after normal child exit. The least-capable default and Codex/Gemini adapters reject sealed grants rather than silently exposing or ignoring them.
