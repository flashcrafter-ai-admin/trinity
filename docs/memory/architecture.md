...
**Sealed operation grants:** synchronous Claude `/api/task` requests may include an optional signed `operation_grant` that is never appended to the model prompt, subprocess argv, or environment. The agent server masks the field with `SecretStr`, rejects async and unsupported-runtime use, and starts Claude through the fixed setuid-root `trinity-task-context` runner. That runner reads only the grant prefix from stdin, validates it through the root-owned packaged operation broker, installs one root-only context under `/run/trinity-task-context`, then drops the Claude child back to UID/GID 1000 with the remaining stdin containing only the task prompt. A non-blocking root lock enforces single-flight use, and the context is removed when the child exits. The least-capable default and Codex/Gemini adapters reject sealed grants rather than silently exposing or ignoring them.
