...
| Date | ID | Change | Flow |
|------|-----|--------|------|
| 2026-07-11 | PKG-001 | Immutable admin-owned platform package registry; templates select an exact package ID and SHA-256 digest and receive fixed-destination Docker named-volume mounts with `RW=false` across create, deploy-local, start, and recreation. | [platform-packages.md](feature-flows/platform-packages.md) |
...
| Immutable Platform Packages | [platform-packages.md](feature-flows/platform-packages.md) | Content-addressed admin packages mounted read-only at fixed agent paths (PKG-001) |
