...
- **Key Features**: Redis-backed atomic lease acquisition; exact fleet-lock digest and candidate-name binding; unique, renewable, generation-safe async and synchronous mutation reservations; pure-ASGI admission across mutating methods, write-bearing OAuth/download GETs, and voice/VoIP/terminal WebSockets; retryable admission for startup reconciliation and scheduler/cleanup loops; persistence-boundary fencing after reservation loss for SQLite, PostgreSQL, and the standalone scheduler; the same authority for detached background writes; and constant-time lease-token verification before atomic whole-lock snapshot comparison.
...
- **Status**: Implemented
- **Description**: Synchronous Claude or server-pinned Codex GPT-5.6 Sol headless tasks may carry a signed `operation_grant` outside the model-visible message, command arguments, and environment.
- **Runtime contract**: The backend dispatches signed work only to the dedicated internal `/api/task/sealed` endpoint, which is absent from legacy images; it never falls back to `/api/task`. The request is rejected for async execution and for runtimes that do not advertise `sealed_operation_grant`. Claude or Codex runs through a fixed setuid-root context runner that validates the grant with the packaged operation broker, installs it under root-controlled `/run` storage for one task, and removes it after the child exits.
- **Public execution contract**: Connector-scoped keys remain MCP/chat principals and cannot submit tasks. A separate `sealed_executor` key is bound to one agent and may reach only `POST /api/agents/{name}/task/sealed` for that agent. The endpoint validates the raw UTF-8 JSON wire before Pydantic normalization, rejects duplicate/extra/coerced fields with one generic response, derives execution-key attribution from the authenticated principal, and requires a durable idempotency claim before dispatch.
- **Runtime containment contract**: Sealed execution is accepted only by a hardened leaf image carrying the exact root-owned runtime profile and boundaries. The non-root model process has no sudo/capability path, receives only the fixed Bash command surface, inherits neither the user's MCP configuration nor resumable session state, and cannot authenticate to Trinity MCP with a sealed-executor key.
- **Security contract**: The native runner accepts only the fixed Claude or Codex executable, never exposes the grant to the model process, and fails closed when the verifier, ownership, mode, signature, runtime profile, root-owned boundary, or request-bound durable idempotency claim is invalid. Codex subscription auth is descriptor-read from the per-agent projection, locked for one turn, copied to a high-entropy ephemeral home for UID 1001 under root-owned execpolicy rules, and persisted back only if source directory and file identities remain unchanged.
