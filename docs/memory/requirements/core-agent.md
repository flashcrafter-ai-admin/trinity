...
- **Security contract**: The native runner accepts only the fixed Claude executable, permits one active context at a time, never exposes the grant to the model process, and fails closed when the verifier, ownership, mode, signature, runtime profile, root-owned boundary, or durable idempotency claim is invalid.
