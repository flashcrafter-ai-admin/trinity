...
- **Key Features**:
  - `scripts/deploy/backup-persistent-state.sh` discovers the compose project by Docker labels, verifies a PostgreSQL custom-format dump for bundled PostgreSQL, uses SQLite's online backup API plus `PRAGMA integrity_check` when SQLite is active, requires a digest-bound snapshot receipt for external PostgreSQL, verifies backend `/data`, requires a nonempty `.env`, and verifies every archived `agent-*-workspace` volume.
  - `scripts/deploy/safe-upgrade.sh` keeps the compose project name stable, requires and validates the completed backup before any build, starts only platform services, waits for backend health, and requires a nonempty `/api/version` response.
  - `.github/workflows/deploy-dev.yml` connects with a tag-scoped Tailscale OAuth client and reaches a dedicated OpenSSH port with a pinned host key, then requests deployment of the exact pushed SHA. The deploy key is restricted to `scripts/deploy/github-actions-safe-deploy.sh`, which rejects arbitrary commands, serializes deploys, verifies the remote `dev` head and an exact clean deployment tree, invokes `safe-upgrade.sh`, verifies runtime provenance, and prunes only superseded clean deployment worktrees.
