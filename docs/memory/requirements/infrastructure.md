...
- **Key Features**:
  - `scripts/deploy/backup-persistent-state.sh` chooses database authority from backend `DATABASE_URL`, proves a fresh PostgreSQL restore or SQLite source/backup integrity, requires signed and provider-verified external snapshots, validates exact required `.env` keys, and compares backend, every Compose named volume, and every agent workspace archive to one paused stable live inventory.
  - `scripts/deploy/safe-upgrade.sh` requires `backup_complete=yes` plus database, writer-pause, volume/workspace coverage, and artifact-inventory evidence; byte-verifies an extracted exact Git tree; rejects escaped or mutable Compose build inputs; activates frozen runtime inputs with `--no-build`; waits for health; and authenticates before requiring `/api/version` to equal the full and short deployed SHA.
  - `.github/workflows/deploy-dev.yml` connects with a tag-scoped Tailscale OAuth client and a pinned host key, then requests the exact pushed SHA. The restricted deploy script serializes deploys; rejects tracked, untracked, ignored, assume-unchanged, and skip-worktree drift; invokes the backup-first immutable-source upgrade; verifies runtime provenance; and prunes only superseded clean worktrees.
