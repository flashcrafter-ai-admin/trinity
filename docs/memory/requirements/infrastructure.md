...
- **Key Features**:
  - `scripts/deploy/backup-persistent-state.sh` chooses database authority from backend `DATABASE_URL`, proves a fresh PostgreSQL restore or SQLite integrity, requires signed and provider-verified external snapshots, validates exact required `.env` keys, and compares restored backend/workspace archives to paused live sources.
  - `scripts/deploy/safe-upgrade.sh` requires `backup_complete=yes`, freezes compose and environment inputs, builds from a write-protected `git archive` of the exact commit, activates with `--no-build`, waits for health, and requires `/api/version` to equal the full and short deployed SHA.
